    ...
    <div className="space-y-8 max-w-[1600px] mx-auto pb-12 transition-all">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 pb-6 border-b border-slate-200">
        <div>
          ...
          <h1 className="text-5xl font-serif-professional tracking-tight text-slate-900  tracking-tight leading-none">{activeIndustry.name} Inventory</h1>
        ...
        <div className="flex flex-wrap items-center gap-4">
           <button 
             ...
             className="px-8 py-2.5 bg-white border border-slate-200 rounded-2xl text-[13px] font-bold  tracking-widest text-slate-900 hover:shadow-lg transition-all active:scale-95 flex items-center gap-2"
           >
           ...
           <button 
             ...
             className="px-10 py-3 bg-blue-600 text-white rounded-2xl text-[13px] font-bold  tracking-[0.3em] shadow-xl shadow-blue-600/30 hover:bg-blue-700 transition-all flex items-center gap-2 active:scale-95"
           >
             <FiPlus /> Add New Item
           </button>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        {inventorySummary.map((stat, i) => (
          <div key={i} className="industrial-card p-12 flex flex-col justify-between bg-white border border-slate-100 shadow-sm rounded-[40px] hover:shadow-xl transition-all group overflow-hidden">
             ...
             <div className="flex justify-between items-start mb-10 relative z-10">
                ...
                <span className="text-[13px] font-bold text-blue-600 bg-blue-50 px-3 py-1 rounded-lg border border-blue-100  tracking-widest">Live Audit</span>
             </div>
             <div className="relative z-10">
                <p className="text-[13px] font-bold text-slate-900  tracking-widest mb-2">{stat.label}</p>
                <h3 className="text-4xl font-serif-professional tracking-tight text-slate-900 tracking-tight leading-none">{stat.value}</h3>
             </div>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-12">
        <div className="xl:col-span-2 industrial-card flex flex-col bg-white border border-slate-100 shadow-sm rounded-[40px] overflow-hidden">
          <div className="p-8 border-b border-slate-50 flex flex-col md:flex-row justify-between items-start md:items-center gap-8 bg-slate-50/20">
            <div>
               <h3 className="text-lg font-bold tracking-widest text-slate-900  ">Product Registry</h3>
            ...
            <div className="flex gap-4 w-full md:w-auto">
               <div className="relative group flex-1 md:w-80">
                  ...
                  <input 
                    ...
                    className="w-full bg-slate-100/50 border-none rounded-2xl py-5 pl-16 pr-6 text-sm font-bold outline-none focus:bg-white focus:ring-4 focus:ring-blue-600/5 transition-all text-slate-900"
                    value={searchTerm}
                    onChange={(e) => setSearchTerm(e.target.value)}
                  />
               </div>
               <button className="p-4 bg-white border border-slate-100 rounded-2xl text-slate-900 hover:text-blue-600 transition-all shadow-sm active:scale-95"><FiFilter size={20} /></button>
            </div>
          </div>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-slate-50/40 text-[12px] font-bold  tracking-widest text-slate-900">
                  <th className="py-5 px-10">Item Name</th>
                  <th className="py-5 px-8 ">Category</th>
                  <th className="py-5 px-8">Valuation</th>
                  <th className="py-5 px-10 text-right">Available Stock / Status</th>
                </tr>
              </thead>
                ...
                {filteredStocks.map((item) => (
                  <tr key={item.id} className="group hover:bg-blue-50/30 transition-all cursor-pointer">
                    ...
                    <td className="py-8 px-8">
                       <span className="text-[13px] font-bold text-slate-900  tracking-widest">{item.category}</span>
                    </td>
                    <td className="py-8 px-8">
        ...
        <div className="space-y-10">
           <div className="industrial-card p-12 bg-blue-600 text-white rounded-[40px] overflow-hidden relative group min-h-[450px] flex flex-col justify-between shadow-2xl">
              ...
              <div className="relative z-10 flex flex-col h-full justify-between">
                 <div>
                    ...
                    <h3 className="text-4xl font-serif-professional tracking-tight text-white  tracking-tight leading-none mb-4">Capacity Vector</h3>
                    <p className="text-blue-50 text-sm font-bold leading-relaxed">Storage utilization is at <span className="text-white font-bold ">Theoretical Peak</span>. Scalability check recommended.</p>
                 </div>
                 <div className="space-y-6">
                    {warehouses.map((wh, i) => (
                      <div key={i} className="space-y-2.5">
                        <div className="flex justify-between items-center text-[13px] font-bold  tracking-widest text-white font-bold">
                          <span>{wh.name}</span>
                 ...
                 <button className="w-full mt-10 py-5 bg-white text-blue-600 font-bold rounded-3xl text-[13px]  tracking-[0.4em] active:scale-95 transition-all shadow-2xl hover:scale-[1.02]">
                    Warehouse Audit
                 </button>
              </div>
           </div>
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="relative w-full max-w-2xl bg-white rounded-[40px] p-12 md:p-14 border border-blue-500/10 shadow-3xl overflow-hidden"
            >
              <div className="absolute top-0 left-0 w-full h-2 bg-blue-600" />
              <div className="flex justify-between items-start mb-12">
                <div>
                  <h2 className="text-4xl font-serif-professional tracking-tight tracking-tight text-slate-900  leading-none">Add New Item</h2>
                  <p className="text-slate-900 font-bold text-[13px] mt-2  tracking-[0.3em]">Supply Chain Ledger Entry</p>
                </div>
                <button 
                  onClick={() => setIsModalOpen(false)}
              ...
              <form onSubmit={addItem} className="space-y-8">
                 <div className="space-y-3">
                   <label className="text-[13px] font-bold  tracking-widest text-slate-900 ml-1">Asset Nomenclature</label>
                   <input 
                     required
                     type="text" 
                     placeholder="e.g. Precision Alloy Cast v4" 
                     className="w-full bg-slate-50 border-none rounded-2xl py-5 px-8 text-base font-bold outline-none focus:ring-4 focus:ring-blue-600/5 text-slate-900 shadow-inner"
                     value={newItem.name}
                 ...
                 <div className="grid grid-cols-2 gap-8">
                   <div className="space-y-3">
                     <label className="text-[13px] font-bold  tracking-widest text-slate-900 ml-1">Domain Classification</label>
                     <select 
                       className="w-full bg-slate-50 border-none rounded-2xl py-5 px-8 text-base font-bold outline-none focus:ring-4 focus:ring-blue-600/5 text-slate-900 appearance-none shadow-inner"
                   ...
                   <div className="space-y-3">
                     <label className="text-[13px] font-bold  tracking-widest text-slate-900 ml-1">Batch Quantum (Quantity)</label>
                     <input 
                       required
                       type="number" 
                       placeholder="0.00" 
                 ...
                 <div className="grid grid-cols-2 gap-8">
                   <div className="space-y-3">
                     <label className="text-[13px] font-bold  tracking-widest text-slate-900 ml-1">Target Cluster (Warehouse)</label>
                     <select 
                       className="w-full bg-slate-50 border-none rounded-2xl py-5 px-8 text-base font-bold outline-none focus:ring-4 focus:ring-blue-600/5 text-slate-900 appearance-none shadow-inner"
                   ...
                   <div className="space-y-3">
                     <label className="text-[13px] font-bold  tracking-widest text-slate-900 ml-1">Market Valuation</label>
                     <input 
                       required
                       type="text" 
                       placeholder="₹0.00" 
