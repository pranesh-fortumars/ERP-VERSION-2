    ...
    <div className="space-y-8 max-w-[1600px] mx-auto pb-12 transition-all">
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 pb-6 border-b border-slate-200">
        <div>
          ...
          <h1 className="text-4xl font-bold text-slate-900  tracking-tight leading-none">{activeIndustry.name} Inventory</h1>
        ...
        <div className="flex flex-wrap items-center gap-4">
           <button 
             ...
             className="px-6 py-2.5 bg-white border border-slate-200 rounded-2xl text-[10px] font-bold  tracking-widest text-slate-900 hover:shadow-lg transition-all active:scale-95 flex items-center gap-2"
           >
           ...
           <button 
             ...
             className="px-10 py-3 bg-blue-600 text-white rounded-2xl text-[10px] font-bold  tracking-[0.3em] shadow-xl shadow-blue-600/30 hover:bg-blue-700 transition-all flex items-center gap-2 active:scale-95"
           >
             <FiPlus /> Add New Item
           </button>
        </div>
        ...
        {inventorySummary.map((stat, i) => (
          <div key={i} className="industrial-card p-10 flex flex-col justify-between bg-white border border-slate-100 shadow-sm rounded-[40px] hover:shadow-xl transition-all group overflow-hidden">
             ...
             <div className="relative z-10">
                ...
                <h3 className="text-3xl font-bold text-slate-900 tracking-tight leading-none">{stat.value}</h3>
             </div>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-10">
        <div className="xl:col-span-2 industrial-card flex flex-col bg-white border border-slate-100 shadow-sm rounded-[40px] overflow-hidden">
          <div className="p-8 border-b border-slate-50 flex flex-col md:flex-row justify-between items-start md:items-center gap-8 bg-slate-50/20">
            <div>
               ...
               <p className="text-[9px] font-bold text-slate-900  tracking-widest mt-1">Multi-Node SKU Identification</p>
            </div>
                  ...
                  <tr key={item.id} className="group hover:bg-blue-50/30 transition-all cursor-pointer">
                    <td className="py-8 px-10">
                      <div className="flex items-center gap-6">
                        ...
                        <div>
                          <p className="text-sm font-bold text-slate-900  leading-none group-hover:text-blue-600 transition-colors">{item.name}</p>
                          <p className="text-[9px] font-bold text-slate-900  tracking-widest mt-1.5 flex items-center gap-2">
                    ...
                    <td className="py-8 px-6">
                       <span className="text-xs font-bold text-slate-900  tracking-tight">{item.price}</span>
                    </td>
                    <td className="py-8 px-10 text-right">
                        <div className="flex items-center gap-8 justify-end">
                           <div className="text-right">
                              <p className="text-[9px] font-bold text-slate-900  tracking-widest mb-1 leading-none">Units</p>
                              <p className="text-sm font-bold text-slate-900 leading-none">{item.stock.toLocaleString()}</p>
                           </div>
                           <span className={`px-5 py-2 rounded-xl text-[9px] font-bold  tracking-widest border border-black/5 shadow-sm min-w-[100px] text-center ${item.color}`}>
                              {item.status}
                           </span>
        ...
        <div className="space-y-10">
           <div className="industrial-card p-10 bg-blue-600 text-white rounded-[40px] overflow-hidden relative group min-h-[450px] flex flex-col justify-between shadow-2xl">
              ...
              <div className="relative z-10 flex flex-col h-full justify-between">
                 <div>
                    ...
                    <h3 className="text-3xl font-bold text-white  tracking-tight leading-none mb-4">Capacity Vector</h3>
                    <p className="text-blue-50 text-xs font-bold leading-relaxed">Storage utilization is at <span className="text-white font-bold ">Theoretical Peak</span>. Scalability check recommended.</p>
                 </div>
                 ...
                 <button className="w-full mt-10 py-5 bg-white text-blue-600 font-bold rounded-3xl text-[10px]  tracking-[0.4em] active:scale-95 transition-all shadow-2xl hover:scale-[1.02]">
                    Warehouse Audit
                 </button>
              </div>
           </div>
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="relative w-full max-w-2xl bg-white rounded-[40px] p-10 md:p-14 border border-blue-500/10 shadow-3xl overflow-hidden"
            >
              <div className="absolute top-0 left-0 w-full h-2 bg-blue-600" />
              <div className="flex justify-between items-start mb-12">
                <div>
                  <h2 className="text-3xl font-bold tracking-tight text-slate-900  leading-none">Add New Item</h2>
              ...
              <form onSubmit={addItem} className="space-y-8">
                 <div className="space-y-3">
                   ...
                   <input 
                     ...
                     className="w-full bg-slate-50 border-none rounded-2xl py-5 px-8 text-sm font-bold outline-none focus:ring-4 focus:ring-blue-600/5 text-slate-900 shadow-inner"
                     value={newItem.name}
                 ...
                 <div className="grid grid-cols-2 gap-8">
                   <div className="space-y-3">
                     ...
                     <select 
                       className="w-full bg-slate-50 border-none rounded-2xl py-5 px-8 text-sm font-bold outline-none focus:ring-4 focus:ring-blue-600/5 text-slate-900 appearance-none shadow-inner"
