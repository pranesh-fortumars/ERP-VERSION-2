...
const DashboardPage = () => {
  ...
  const stats = [
    { label: 'Total Revenue', value: activeIndustry.stats.revenue, change: '+12.5%', isUp: true, icon: <FiDollarSign className="w-5 h-5" />, color: 'indigo' },
  ...
  return (
    <div className="space-y-8 max-w-[1600px] mx-auto pb-12 transition-all">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 pb-6 border-b border-slate-100">
        <div>
           <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-50 text-indigo-600 text-[14px] font-serif-professional tracking-wide  tracking-widest mb-4 border border-indigo-100 font-black">
          ...
          <p className="text-slate-950 font-bold text-xl mt-3 flex items-center gap-2">
            <FiActivity className="text-indigo-600" /> Real-time Node Telemetry & Orchestration Dashboard for {activeIndustry.location}
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-4">
           ...
           <button 
             ...
             className="bg-white border border-slate-200 text-[14px] font-serif-professional tracking-wide  tracking-widest px-6 py-3 rounded-[24px] hover:shadow-lg transition-all active:scale-95 flex items-center gap-2"
           >
             {isExporting ? <div className="w-3 h-3 border-2 border-indigo-600/30 border-t-indigo-600 rounded-full animate-spin-slow" /> : <FiDownload />} {isExporting ? 'Generating...' : 'PDF Export'}
           </button>
           <button 
             onClick={() => setIsModalOpen(true)}
             className="bg-indigo-600 text-white px-14 py-3 rounded-[24px] text-[14px] font-serif-professional tracking-wide  tracking-[0.3em] shadow-xl shadow-indigo-600/30 hover:bg-indigo-700 active:scale-95 transition-all flex items-center gap-2"
           >
             <FiPlus /> Log Incident
           </button>
        </div>
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.15 }} key={i} className="industrial-card animate-fade-up p-10 flex flex-col justify-between bg-white border border-slate-100 shadow-sm rounded-[40px] hover:shadow-2xl transition-all duration-700 relative overflow-hidden group">
             ...
             <div className="flex justify-between items-start mb-10 relative z-10">
                <div className={`w-14 h-14 rounded-[24px] flex items-center justify-center text-white shadow-xl group-hover:rotate-12 transition-transform text-3xl ${
                  stat.color === 'indigo' ? 'bg-indigo-600 shadow-indigo-600/20' :
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-12 relative z-10 gap-6">
            ...
            <div className="flex gap-3">
               {['Revenue Flux', 'Batch Yield'].map((m, i) => (
                 <button key={i} className={`text-[12px] font-serif-professional uppercase tracking-widest px-6 py-2.5 rounded-[24px] transition-all ${i === 0 ? 'bg-indigo-600 text-white shadow-xl shadow-indigo-600/30' : 'bg-slate-50 text-slate-950 border border-slate-100 hover:bg-white'}`}>
                   {m}
                 </button>
               ))}
            </div>
        ...
        <div className="flex flex-col gap-8">
           <div className="industrial-card animate-fade-up p-10 bg-indigo-600 text-white flex-1 flex flex-col justify-between relative overflow-hidden group shadow-2xl rounded-[40px]">
              ...
              <div className="relative z-10">
                 ...
                 <p className="text-indigo-50 text-xl font-bold leading-relaxed">Infrastructure status is currently <span className="text-white font-black underline decoration-indigo-400 underline-offset-4">Optimal</span> across all nodes.</p>
              </div>
              <div className="pt-10 border-t border-white/10 flex items-end justify-between relative z-10">
                 <div>
                    <p className="text-[14px] font-black text-indigo-200 uppercase tracking-[0.3em] mb-2">Cluster Util</p>
           ...
           <div className="industrial-card animate-fade-up p-10 bg-white border border-slate-100 shadow-sm rounded-[40px]">
              <h3 className="text-[13px] font-black text-slate-950 uppercase tracking-[0.3em] mb-8 flex items-center gap-3">
                 <FiShield className="text-indigo-600" /> Security Perimeter
              </h3>
              <div className="space-y-6">
                 {[
                   { label: 'Gateway Firewall', status: 'Active', color: 'emerald' },
                   { label: 'Cloud Relays', status: 'Optimal', color: 'indigo' },
                   ...
                   <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.15 }} key={i} className="flex items-center justify-between group">
                     <span className="text-[14px] font-black text-slate-950 uppercase tracking-widest group-hover:text-indigo-600 transition-colors">{item.label}</span>
           ...
           {incidents.map((inc) => (
             <div key={inc.id} className="p-8 flex items-center justify-between hover:bg-slate-50/50 transition-all cursor-pointer group">
                <div className="flex items-center gap-8">
                   ...
                   <div>
                      <p className="text-3xl font-serif-professional tracking-tight text-slate-950 uppercase tracking-tight group-hover:text-indigo-600 transition-colors">{inc.type}</p>
        ...
        {isModalOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="relative w-full max-w-xl bg-white rounded-[40px] p-8 border border-indigo-500/20 shadow-3xl overflow-hidden"
            >
              ...
              <form onSubmit={handleAddIncident} className="space-y-10">
                <div className="space-y-4">
                  ...
                  <input required type="text" placeholder="e.g. Node Cluster Desync" className="w-full bg-slate-50 border-none rounded-[32px] py-4 px-8 text-xl font-black outline-none focus:ring-4 focus:ring-indigo-600/5 text-slate-950" 
                ...
                <div className="grid grid-cols-2 gap-8">
                  <div className="space-y-4">
                    ...
                    <select className="w-full bg-slate-50 border-none rounded-[32px] py-4 px-8 text-xl font-black outline-none focus:ring-4 focus:ring-indigo-600/5 text-slate-950 appearance-none"
                ...
                <button type="submit" className="w-full mt-6 py-4 bg-indigo-600 text-white rounded-[32px] font-black text-base uppercase tracking-[0.4em] shadow-2xl shadow-indigo-600/30 hover:bg-indigo-700 transition-all hover:scale-[1.02] active:scale-95 flex items-center justify-center gap-3">
          ...
          <div className="fixed inset-0 z-[110] flex items-center justify-end p-0 md:p-6">
            ...
            <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'spring', damping: 25, stiffness: 200 }} className="relative w-full max-w-2xl h-full md:h-[calc(100vh-3rem)] bg-white rounded-none md:rounded-[40px] p-8 border-l border-slate-200 shadow-3xl overflow-y-auto no-scrollbar">
              ...
              <div className="space-y-8">
                 {[1,2,3,4,5,6].map((log) => (
                   <div key={log} className="p-8 bg-slate-50 rounded-[40px] border border-transparent hover:border-indigo-600/10 transition-all group">
                      ...
                      <p className="text-base font-bold text-slate-950 leading-relaxed uppercase tracking-tight">
                         User <span className="text-indigo-600">Admin_Root</span> initiated metadata sync for Node cluster {log}A. Security check: <span className="text-emerald-500 underline decoration-emerald-200">Passed</span>.
                      </p>
                   </div>
                 ))}
              </div>
