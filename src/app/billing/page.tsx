      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {[
          ...
          >
            ...
            <div>
              <p className="text-[10px] font-black text-slate-900 dark:text-slate-900 uppercase tracking-[0.3em] leading-none mb-3">{stat.label}</p>
          ...
          <div className="flex flex-wrap justify-center gap-3">
            {['All', 'Paid', 'Pending', 'Overdue'].map(s => (
              <button 
                key={s} 
                onClick={() => setFilter(s)}
                className={`px-6 py-2.5 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all ${filter === s ? 'bg-blue-600 text-white shadow-xl shadow-blue-600/30' : 'bg-white  text-slate-900 text-slate-900 border border-black/5 hover:bg-slate-100 active:scale-95 shadow-sm'}`}
              >
                {s}
              </button>
            ))}
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50/30 /30">
                <th className="px-10 py-6 text-[10px] font-black text-slate-900 dark:text-slate-900 uppercase tracking-[0.2em]">Asset Descriptor</th>
                <th className="px-10 py-6 text-[10px] font-black text-slate-900 dark:text-slate-900 uppercase tracking-[0.2em]">Quantum (INR)</th>
                <th className="px-10 py-6 text-[10px] font-black text-slate-900 dark:text-slate-900 uppercase tracking-[0.2em]">Maturity Target</th>
                <th className="px-10 py-6 text-[10px] font-black text-slate-900 dark:text-slate-900 uppercase tracking-[0.2em] text-center">Lifecycle State</th>
              </tr>
            </thead>
                  ...
                  >
                    <td className="px-10 py-8">
                      <div className="flex items-center gap-6">
                        <div className="w-14 h-14 rounded-2xl bg-slate-100  flex items-center justify-center text-slate-900 group-hover:text-blue-500 transition-colors shadow-inner border border-black/5">
                          <FiFileText size={24} />
                        </div>
                        <div>
                          ...
                          <p className="text-[10px] text-slate-900 dark:text-slate-900 mt-1 uppercase font-black tracking-[0.3em] flex items-center gap-2">
                    ...
                    <td className="px-10 py-8">
                       ...
                       <p className="text-[9px] font-black text-slate-900 uppercase tracking-widest mt-1">Settlement Value</p>
                    </td>
                    <td className="px-10 py-8">
                       <p className="text-sm font-black text-slate-900 text-slate-900 tracking-tight uppercase">{inv.dueDate}</p>
                       <p className="text-[9px] font-black text-slate-900 uppercase tracking-widest mt-1">Fiscal Deadline</p>
                    </td>
            ...
            >
               <div className="flex justify-between items-start mb-12">
                <div>
                  ...
                  <p className="text-slate-900 font-black text-[10px] mt-2 uppercase tracking-[0.3em]">Corporate Billing Infrastructure</p>
                </div>
                <button 
                  onClick={() => setIsModalOpen(false)}
                  className="p-4 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-2xl text-slate-900 transition-colors active:scale-95"
                >
              ...
              <form onSubmit={addInvoice} className="space-y-8">
                <div className="space-y-3">
                  <label className="text-[10px] font-black uppercase tracking-widest text-slate-900 ml-1">Client/Entity Portfolio</label>
                  <input 
                    required
                    type="text" 
                ...
                <div className="grid grid-cols-2 gap-6">
                  <div className="space-y-3">
                     <label className="text-[10px] font-black uppercase tracking-widest text-slate-900 ml-1">Settlement Quantum (INR)</label>
                     <input 
                       required
                       type="number" 
                       placeholder="0.00"
                  ...
                  <div className="space-y-3">
                     <label className="text-[10px] font-black uppercase tracking-widest text-slate-900 ml-1">Maturity Target Date</label>
                     <input 
                       required
                       type="date" 
                       className="w-full bg-slate-50  border-none rounded-2xl py-4.5 px-6 text-sm font-black outline-none focus:ring-2 focus:ring-blue-500/10 text-slate-900"
                ...
                <div className="space-y-3">
                   <label className="text-[10px] font-black uppercase tracking-widest text-slate-900 ml-1">Corporate GSTIN Identifier</label>
                   <input 
                     required
                     type="text" 
