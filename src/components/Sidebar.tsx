    ...
    <>
      ...
      <div className="w-72 sidebar-standard p-6 hidden lg:flex flex-col h-screen fixed left-0 top-0 z-50 border-r border-blue-500/10 shadow-2xl bg-white">
        ...
        <div className="flex items-center justify-between mb-8 px-2 mt-4 group">
          <div className="flex items-center gap-3">
             ...
             <div>
               ...
               <p className="text-[8px] font-black text-slate-400 uppercase tracking-[0.3em] leading-none mt-1">Industrial Command</p>
             </div>
          </div>
        </div>
        ...
        <div className="mb-8 relative">
           <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-3 mb-3">Enterprise Node</p>
           <button 
           ...
           >
              <div className="flex items-center gap-3 overflow-hidden text-left">
                 ...
                 <div className="overflow-hidden">
                    ...
                    <p className="text-[8px] font-bold text-slate-400 uppercase tracking-[0.1em]">{activeInstance.location}</p>
                 </div>
              </div>
              <FiChevronDown className={`text-slate-400 transition-transform duration-300 ${isInstanceSelectorOpen ? 'rotate-180 text-blue-600' : ''}`} />
           </button>
             ...
             {isInstanceSelectorOpen && (
               ...
               >
                 {instances.map((instance) => (
                   ...
                   >
                     <div className={`w-8 h-8 rounded-lg flex items-center justify-center text-[10px] font-black transition-colors ${activeInstance.id === instance.id ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-400 group-hover:bg-blue-600 group-hover:text-white'}`}>
                        {instance.id.split('-')[1]}
                     </div>
                     <div className="overflow-hidden">
                        ...
                        <p className="text-[8px] font-bold text-slate-400 uppercase">{instance.location}</p>
                     </div>
                   </button>
                 ))}
              ...
              return (
                <li key={item.href}>
                  ...
                  >
                    ...
                    <span className={`text-xl transition-all duration-500 ${isActive ? 'text-blue-600 scale-110 drop-shadow-[0_0_8px_rgba(37,99,235,0.2)]' : 'text-slate-400 group-hover:text-blue-600 group-hover:scale-110'}`}>
                      {item.icon}
                    </span>
                    <span className={`text-[11px] font-black uppercase tracking-[0.15em] shrink-0 ${isActive ? 'text-blue-600' : 'text-slate-600'}`}>{item.label}</span>
                  </Link>
                </li>
              );
            })}
            ...
            <>
              ...
              >
                 ...
                 <div className="mb-10 text-center">
                    ...
                    <p className="text-[9px] text-slate-400 font-bold uppercase mt-1">Cluster {activeInstance.id}</p>
                 </div>
                ...
                <div className="mt-6 p-4 bg-slate-50 rounded-[24px] border border-slate-100 flex items-center gap-4">
                    ...
                    <button className="ml-auto w-8 h-8 rounded-lg bg-white border border-slate-200 flex items-center justify-center text-slate-400">
                       <FiSettings size={14} />
                    </button>
                </div>
              </motion.div>
               ...
               >
                  <div className="flex justify-between items-start mb-8">
                     <div>
                        ...
                        <p className="text-[9px] font-black text-slate-400 mt-2 uppercase tracking-[0.2em]">Global EPR Deployment Switcher</p>
                     </div>
                  ...
                  <div className="space-y-4">
                     {instances.map(instance => (
                        ...
                        >
                           <div className={`w-12 h-12 rounded-2xl flex items-center justify-center font-black text-lg ${activeInstance.id === instance.id ? 'bg-blue-600 text-white shadow-xl shadow-blue-600/30' : 'bg-slate-100 text-slate-400'}`}>
                              {instance.id.split('-')[1]}
                           </div>
                           <div className="text-left overflow-hidden">
                              <p className="font-black text-slate-900 uppercase text-sm leading-none mb-1 truncate">{instance.name}</p>
                              <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{instance.location}</p>
                           </div>
                        </button>
                     ))}
                  </div>
