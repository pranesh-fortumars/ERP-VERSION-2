  ...
  if (active && payload && payload.length) {
    return (
      <div className="p-3 bg-white/95 /95 backdrop-blur-md border border-slate-200 border-slate-200 rounded-[24px] shadow-xl">
        <p className="text-[14px] font-serif-professional tracking-wide  tracking-widest text-slate-950 mb-2">{label}</p>
        {payload.map((pld: any, index: number) => (
          <div key={index} className="flex items-center gap-3">
             ...
             <div className="flex-1 flex justify-between gap-6">
                <span className="text-base font-bold text-slate-950 text-slate-950">{pld.name}:</span>
