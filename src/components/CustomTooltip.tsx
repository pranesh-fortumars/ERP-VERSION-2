  ...
  if (active && payload && payload.length) {
    return (
      <div className="p-3 bg-white/95 /95 backdrop-blur-md border border-slate-200 border-slate-200 rounded-2xl shadow-xl">
        <p className="text-[10px] font-black uppercase tracking-widest text-slate-900 mb-2">{label}</p>
        {payload.map((pld: any, index: number) => (
          <div key={index} className="flex items-center gap-3">
             ...
             <div className="flex-1 flex justify-between gap-6">
                <span className="text-xs font-bold text-slate-900 text-slate-900">{pld.name}:</span>
